...
- **취소가 원형 복원 안 되던 원인:** 부스 편집 중 꼭짓점/곡선을 드래그할 때마다 `onBoothShapeChange`(→`updateBoothShape`)가
  **`project.boothConfig`를 즉시 수정**해서, 취소해도 되돌릴 원본이 남지 않았음.
- **draft 방식으로 재설계:** 편집 진입 시 폴리곤+곡선을 **로컬 draft(editPoints/editCurves)**로 복제. 드래그·삭제는 **draft만** 수정,
  `boothConfig`는 건드리지 않음(자동저장도 draft 저장 안 함). 캔버스 하단 액션바 추가:
  - **[완료]** → draft를 `updateBoothShape`로 **1회 적용**(Undo History 1개), 자동 저장.
  - **[취소]**(또는 ESC) → draft 폐기 → 편집 시작 전 형태 **완전 복원**(History에 안 남김).
- **[사각형으로 초기화]:** 확인창 후 현재 Width/Depth 유지한 정확한 직사각형 4점 + 곡선 제거 → **`boothShape='rectangle'`**(축 정렬 사각형 자동 판정). 집기·디자인·제품·벽색·오픈면 모두 유지.
- **[모든 곡선 제거]:** polygonPoints 유지, edgeCurves만 제거(직선화), 크기 유지.
- **Undo/Redo:** 점 이동·곡선·초기화·크기 변경 모두 **완료 시 1개**만 쌓임(드래그 중 프레임마다 X). 취소는 History 미기록.
- *직각(Shift) 스냅은 이번 범위에서 제외.*

**곡선 다각형 치수 일치 · 집기 W×D×H (v1.2.4)**
- **곡선 다각형 치수 불일치 수정(입력 3000인데 2D 3003):** 원인 = `getBoothBounds`(3D/출력/벽/면적)는 **원본 꼭짓점 bbox**,
  2D 평면도는 **곡선 반영 외곽선 bbox** 를 써서 곡선 부스에서 값이 갈라짐.
  → `getBoothBounds` 를 **외곽선(getBoothOutline) bbox** 기준으로 통일(사각형·비곡선은 무회귀). `updateBoothSize` 도
  외곽선 bbox 로 스케일·`widthMm/depthMm` 재계산 → **숫자 필드 = 2D = 3D = 출력 = 면적 완전 일치**(단위테스트: 저장값==getBoothBounds).
  면적(상단·상태바)도 외곽선 기준으로 통일.
- **집기 치수 가로×세로×높이 통일:** 공통 포맷터 `formatFixtureDims(w,d,h)` 도입 → 2D 평면도 라벨·3D 미리보기·PNG/PDF 출력 모두
  `1275×500×930` 형식(높이 없으면 `1275×500`, 임의 0 표시 안 함). 파란 배경·흰 글씨 디자인 유지. 높이 소스 = 집기 정의 heightMm(커스텀/GLB 실물높이 반영).
- v1.2.3 다중 필드 안전(함수형 setProject)·편집 중 리셋 방지 유지.

**부스 크기 변경 전체 동기화 (v1.2.3)**
- **원인:** `updateBoothSize` 가 **stale 클로저의 `project`** 로 `{...project}` 를 만들어, 가로→세로→높이를
  연속 수정하면 앞서 커밋한 값이 옛 값으로 덮어써질 수 있었음(다중 필드 편집 시 "일부 이전 값 유지").
  또 `BoothSizeFields` 의 동기화 useEffect 가 한 필드 커밋 시 편집 중이던 다른 필드를 리셋.
- **수정:** ① `updateBoothSize` 를 **함수형 `setProject(prev => …)`** 로 변경 → 항상 최신 상태 기준 계산(다중 필드 안전).
  ② 다각형은 스케일 후 **`widthMm/depthMm` 를 실제 폴리곤 bbox 로 재계산** → 숫자 필드와 `getBoothBounds`(2D/3D/벽/출력/면적)가 정확히 일치(단일 소스).
  ③ `BoothSizeFields` 에 **포커스 가드** 추가 → 편집 중 필드는 재동기화 스킵(입력 유실 방지).
- **동기화 대상 검증:** 상단 입력 · 2D 편집/치수선 · 면적 · (아키텍처상) 벽면·3D·PNG/PDF 모두 `getBoothBounds`/`getBoothPolygon`(순수 함수, boothConfig 직접 참조)로 계산 — 캐시/복제 없음. 저장 버튼 없이 즉시 반영, Undo/Redo 유지.
  - *VMD 보드는 부스와 독립된 사이즈(설계상 별개)라 부스 크기 변경의 영향을 받지 않음.*

**부스 크기 입력 수정 · 2D 부스 라벨 제거 (v1.2.2)**
- **부스 크기 입력이 안 되던 원인:** `BoothSizeFields` 가 **다각형 부스에서 `disabled`** 였음(사각형만 편집).
  → **모든 부스에서 편집 가능**하게 수정. 다각형은 `updateBoothSize` 가 **polygonPoints(+edgeCurves)를 bbox 기준 비례 스케일**해 형태를 유지하며 크기 변경. 사각형은 직접 설정.
  숫자만 입력·Enter 적용·Escape 원복·자동저장·Undo/Redo(히스토리 boothConfig 반영)·2D/3D/벽/치수 즉시 갱신, 부스가 작아져도 집기 자동 삭제 안 함.
- **2D 파란 "부스 W×D mm" 라벨 제거:** 평면도·PNG/PDF 출력에서 파란 배경 라벨 삭제. **부스 외곽 치수선 + 숫자(가로/세로 mm)는 그대로 유지**(스타일 변경 없음). 3D 미리보기 라벨은 정책대로 유지.

**중앙 리사이즈 재설계 · 접기/확장 · 부스 크기 인라인 (v1.2.1)**
- **상단 패널 잘림 해결:** 원인 = 상단 박스가 `height 고정 + overflow` 라 내용이 리플로우 없이 잘림 + 툴바가 클러스터 맨 아래.
  → **툴바를 상단 클러스터 맨 위로 이동**(높이를 줄여도 툴바 항상 보임) + 정보 바 컴팩트화. flex column(상단 height + 캔버스 flex:1) 구조로 실제 리사이즈(겹침 없음).
- **드래그 UX:** `requestAnimationFrame` 로 부드럽게, localStorage 저장은 **드래그 종료 시**. Divider hover/drag 파란 강조·row-resize·넓은 히트·더블클릭 기본 복원·방향키.
- **상단 접기(#7):** Divider 우측 chevron 으로 상단을 완전 최소화(0)→캔버스 최대, 다시 누르면 마지막 높이 복원. 상태 localStorage.
- **작업 공간 확장(#8):** 툴바에 버튼 추가(동일 토글). 접힘 시엔 Divider chevron 으로 복원.
- **부스 크기 인라인 편집(#5):** 상단 정보 바에서 가로·세로·높이(mm) 직접 입력 — Enter 적용/Escape 취소, 자동저장,
  **Undo/Redo(히스토리 boothConfig 반영)**, 2D/3D 즉시 반영, 부스 축소 시 집기 자동 삭제 안 함. (다각형은 부스 편집 사용)
- **드롭다운 잘림 방지(#9):** MUI Menu/Popover 는 body 로 Portal 되어 상단 overflow 와 무관하게 표시됨(확인).
- 치수 규칙(부스 외곽 치수 항상 표시)·캔버스 줌 유지는 v1.2.0 유지.

**중앙 세로 리사이즈 · 부스 크기 입력 · 치수 규칙 정정 (v1.2.0)**
- **중앙 작업영역 세로 리사이즈:** 상단 클러스터(부스 정보·힌트·툴바)와 메인 캔버스 사이에 가로 Divider 추가.
  위·아래 드래그로 상단 높이 조절(VSCode 하단 패널 스타일) — `row-resize` 커서, hover/drag 파란 강조,
  넓은 히트영역, 더블클릭 기본 복원, 키보드(방향키) 조절. 최소 상단 96px·캔버스 300px, 화면 축소 시 자동 clamp.
  높이는 localStorage(`blp:centerTopH:plan` / `:wall`) 저장. 설정에 **중앙 패널 배치 초기화** 버튼.
  캔버스는 남은 공간에 맞춰 즉시 리사이즈(ResizeObserver), **리사이즈 시 수동 줌 유지 + 화면 중심 보존**(부스가 잘리지 않음).
- **부스 크기 직접 입력:** 설정에서 가로·세로·높이(mm)를 직접 수정(사각형 부스). 즉시 반영·자동 저장.
  (다각형 부스는 기존 '부스 편집' 꼭짓점 드래그 사용)
- **치수 규칙 정정:** `[치수]` OFF 여도 **부스 외곽 치수(가로/세로 치수선·값·전체 크기 라벨)는 항상 표시**.
  OFF 시 숨김 = 집기 치수·집기 간격·벽 치수(선). `[집기명]`은 치수와 독립.

**치수 토글 통일 · 라벨 일원화 · 패널 리사이즈 (v1.1.9)**
- **[치수] 토글 2D·3D 완전 통일:** 토글 하나로 부스·벽·집기 치수 + 치수선 + 간격 치수를 2D 평면도와
  3D 미리보기에서 **모두** ON/OFF. 3D 미리보기의 별도 '사이즈 표기'·'라벨' 서브토글 제거(상단 [집기명]·[치수]로 통합).
  편집기와 3D 다이얼로그의 토글이 같은 상태를 공유(single source).
- **집기 이름은 치수와 분리:** [집기명] 토글은 독립 유지 — 치수 OFF·이름 ON 이면 이름만 표시.
- **치수 라벨 파란색 일원화(#5):** 부스·벽·집기·간격 라벨 모두 동일 스타일(파란 배경 `#2563eb`·흰 글자·동일 radius/padding/shadow). 2D·3D·출력 공통.
- **우측 속성 패널 폭 리사이즈(#3):** 좌측 라이브러리와 동일한 수준의 UX — 넓은 히트영역, hover/drag 시 Divider 강조,
  더블클릭 기본폭 복원, localStorage 유지, 화면 축소 시 자동 보정(VSCode/Figma 스타일).

**치수/라벨 마감 · 벽 색상 정확도 (v1.1.8)**
- **[치수] 토글:** 툴바(평면도)에 `치수` 스위치 추가(localStorage 유지). OFF 시 부스 치수선·부스 라벨·
  집기 사이즈 라벨·집기 간격선을 일괄 숨김(사용자 치수선 주석은 유지). 2D·3D·PNG/PDF 출력에 반영.
- **통일 치수 라벨(파란 배경·흰 글자·라운드·그림자):** 부스 전체 치수를 바닥 중앙 하단에 `부스  W × D mm`
  파란 라벨로 표시(2D+출력). 집기는 **이름 ↑(상단) · 사이즈 W×D ↓(하단, 파란 라벨)** 규칙으로 통일(#6).
- **벽 색상 정확도(3D):** 사용자가 지정한 벽 색은 3D에서 조명 셰이딩을 건너뛰고 **HEX 원본 그대로** 렌더
  → `#ffffff`=순백 · `#000000`=순검정 · `#ff0000`=빨강 정확 표현(기존엔 조명 곱으로 회색/베이지). 미지정 벽은 음영 유지.
- **3D 벽 치수 라벨 개선:** 파란 통일 라벨 · 폰트 확대 · 벽 상단 바깥으로 offset · **겹치면 자동 회피(스킵)** ·
  줌/카메라 회전 무관 고정 크기(화면 공간 billboard).
- *제한/보류: 집기 편집 패널 세로 리사이즈(#3, 대상 패널 확인 필요) · 설정 아코디언 그룹화(#8)는 다음 버전.*
